...
def create_cell_graph(protein_mat,neighbors):
    graphs= []
    edge_indices = []
    for neighbor_idxs in neighbors:
        # Node features for the current cell graph
        nodes = torch.tensor(protein_mat.iloc[neighbor_idxs, :].values, dtype=torch.float)
        
        nodes = torch.log(nodes+1e-5)
        # Edges for the current cell graph: connect the first node to all other nodes
        source_nodes = []
        target_nodes = []
        for i in range(1, len(neighbor_idxs)):  # Start from 1 to skip the first node itself
            source_nodes.append(0)  # The first node is the source
            target_nodes.append(i)  # Each subsequent node is a target

        # Optionally, add reverse edges to make connections bidirectional
        for i in range(1, len(neighbor_idxs)):
            source_nodes.append(i)  # Each subsequent node is now a source
            target_nodes.append(0)  # The first node is the target
