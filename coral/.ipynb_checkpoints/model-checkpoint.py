...
class CORAL_model(nn.Module):
    def __init__(self, visium_dim, codex_dim, cell_type_dim, latent_dim, hidden_channels, v_dim,  eps=1e-10):
        ...
        #self.gcn = GCNConv(latent_dim, hidden_channels, add_self_loops=False)
    ...
    def encode(self, x_combined, cell_type, edge_index):
        ...
        visium_h = self.encoder_visium(torch.cat((x_combined[:, :self.visium_dim], cell_type), dim=1))#self.encoder_visium(x_combined[:, :self.visium_dim])#
        ...
        #visium_mu = self.gcn(visium_mu, edge_index, edge_weight=edge_weights)
        ...
        #codex_mu = self.gcn(codex_mu, edge_index, edge_weight=edge_weights)
    ...
    def forward(self, batch):
        ...
        #print(deconv_x)
        q_xi_dist = NegBinom(torch.exp(visium_library)*deconv_x+ self.eps,torch.exp(self.px_r_sc))
